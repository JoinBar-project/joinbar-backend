...
const createLinePayment = async (req, res) => {
  try {
    ...
    if (!order) {
      return res.status(404).json({ error: '找不到待付款訂單', code: 'ORDER_NOT_FOUND' });
    }

    if (order.paymentId && order.paymentMethod === 'linepay') {
      const statusCheck = await checkPaymentStatus(order.paymentId);
    ...
    const paymentData = {
      ...
      packages: [{
        ...
        amount: order.totalAmount,
        name: '活動票券',
        products: orderItemsList.map(item => ({
          id: `product_${item.id}`,
...
const confirmLinePayment = async (req, res) => {
  try {
    ...
    const confirmResult = await confirmPayment(transactionId, order.totalAmount, 'TWD');
...
const checkLinePaymentStatus = async (req, res) => {
  try {
    ...
    if (!order) {
      return res.status(404).json({ error: '找不到訂單', code: 'ORDER_NOT_FOUND' });
    }

    let linePayStatus = null;

    if (order.paymentId && order.paymentMethod === 'linepay') {
      const statusResult = await checkPaymentStatus(order.paymentId);
...
const refundLinePayment = async (req, res) => {
  try {
    ...
    if (req.user.role !== 'admin') {
      return res.status(403).json({ error: '無權限執行退款', code: 'FORBIDDEN' });
    }
    ...
    if (order.status !== 'confirmed' && order.status !== 'paid') {
      return res.status(400).json({ error: '只能退款已確認的訂單', code: 'INVALID_ORDER_STATUS' });
    }

    if (!order.paymentId || order.paymentMethod !== 'linepay') {
      return res.status(400).json({ error: '非 LINE Pay 付款，無法退款', code: 'INVALID_PAYMENT_METHOD' });
    }

    const refundResult = await refundPayment(order.paymentId, order.totalAmount, 'TWD');
    ...
    await db.transaction(async (tx) => {
      ...
      await tx.delete(userEventParticipationTable).where(and(
