...
const envSchema = z
  ...
  .superRefine((data, ctx) => {
    if (
      data.ADMIN_DEFAULT_PASSWORD !== undefined &&
      data.ADMIN_DEFAULT_PASSWORD.length < data.APPLICATION_PASSWORD_MIN_LENGTH
    ) {
      ctx.addIssue({
        code: z.ZodIssueCode.too_small,
        minimum: data.APPLICATION_PASSWORD_MIN_LENGTH,
        origin: 'string',
        inclusive: true,
        path: ['ADMIN_DEFAULT_PASSWORD'],
        message: `Too small: expected string to have >=${data.APPLICATION_PASSWORD_MIN_LENGTH} characters`,
      });
    }
  });
