...
- **THEN** 建立重設 token，透過 `SendEmailPort` 寄出連結，回傳 HTTP 200
...
- **THEN** 更新密碼，標記 token `usedAt`，回傳 HTTP 200
...
當 `APPLICATION_IS_LOGOUT_AFTER_PASSWORD_RESET=true` 時，系統 SHALL 將使用者所有現有 token 設為無效。

#### Scenario: 重設後清除 session

- **WHEN** 密碼重設成功且 `APPLICATION_IS_LOGOUT_AFTER_PASSWORD_RESET=true`
